        ...
        <form action="{{ route('admin.arsip.update', $arsip->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <select id="jenis_surat_id" 
                    ...
                    @foreach($jenisSurat as $jenis)
                        <option value="{{ $jenis->id }}" 
                                {{ old('jenis_surat_id', $arsip->jenis_surat_id) == $jenis->id ? 'selected' : '' }}>
                            {{ $jenis->nama_jenis }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" 
                    id="date" 
                    name="date" 
                    value="{{ old('date', $arsip->created_at->format('Y-m-d')) }}"
            ...
            @if($arsip->surat_diminta)
                <div>
                    ...
                    <div class="flex items-center space-x-3 p-3 bg-gray-50 rounded-md">
                        ...
                        <span class="text-sm text-gray-700">{{ basename($arsip->surat_diminta) }}</span>
                        <a href="{{ asset('storage/' . $arsip->surat_diminta) }}" 
                           target="_blank" 
            ...
            <div>
                <label for="surat_diminta" class="block text-sm font-medium text-gray-700 mb-2">
                    Upload Dokumen Baru 
                    <span class="text-gray-500 text-xs">(Opsional - biarkan kosong jika tidak ingin mengubah dokumen)</span>
                </label>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Update Arsip
                </button>
            </div>
        </form>
    </div>
